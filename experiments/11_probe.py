...
# EXP_D5 — two fixes to D4. ~20 min on a T4.  Output: exp_D5_probe.json
#
# D4 RAN CORRECTLY AND ITS STYLE RESULT IS STILL NOT USABLE, because of a bug I
# introduced in the cluster id:
#
#   style_unit = f"{m['phr']}"        # phrasing INDEX only
#
# Phrasing index 5 exists in all five dimensions, so those five distinct cue pairs
# collapsed into ONE cluster. D4 reported "1080 items from 3 held-out phrasings" --
# 3 units, not 15. With 3 units the bootstrap intervals are far too wide and the
# permutation test has 2^3 = 8 sign patterns, so its smallest reachable p is about
# 0.125. That is why every style p came back 0.228, 0.498, 0.749 or 1.000: those
# are the only values that test can produce. It was not a null, it was a test with
# no power.
#
#   FIX: unit = (dimension, phrasing) -> 15 held-out units.
#
# SECOND FIX, the urgency control. Base AUC ran 0.419-0.592 -- barely above chance,
# below it at three layers. A 1536-dimension probe trained on 18 independent
# scenarios memorises. Now: average every cue's activation within a scenario first
# (36 vectors, not 2880 correlated ones) and regularise hard (C=0.05).
#
# BE CLEAR ABOUT THE CEILING: this corpus has 50 scenarios, 18 of them urgent. A
# scenario-level control can never have more than 18 units here. Expect it to stay
# weak, and report it as an underpowered check rather than as evidence of
# selectivity. The behavioural selectivity test -- clinical sensitivity unchanged
# at +2.7% -- is the load-bearing one, and you already have it.
#
        ...
        for pi, (sh, sl) in enumerate(prs):
            for slab, cue in ((1, sh), (0, sl)):
                for sid, hist, ulab, _ in scen:
                    ...
                    meta.append(dict(dim=dim, phr=pi, style=slab, sid=sid, urg=ulab))
        print(f"   {dim} done", flush=True)
    return {L: np.array(v) for L, v in Xf.items()}, {L: np.array(v) for L, v in Xc.items()}, meta

def probe(X, y, tr, te, C=1.0):
    sc = StandardScaler().fit(X[tr])
    clf = LogisticRegression(max_iter=3000, C=C, random_state=0).fit(sc.transform(X[tr]), y[tr])
    Z = sc.transform(X[te])
    return clf.decision_function(Z), clf.predict_proba(Z)[:, 1]

def probe_by_scenario(X, sid_arr, y_arr, tr_ids, te_ids, C=0.05):
    """Urgency lives at the SCENARIO level, so average every cue's activation
    within a scenario first: 36 vectors, not 2880 correlated ones. With only 18
    training units against 1536 dimensions the probe must also be regularised
    hard, or it memorises. C=0.05, and the result is reported as underpowered
    whatever it says."""
    ids = sorted(set(sid_arr))
    V = np.stack([X[sid_arr == i].mean(0) for i in ids])
    Y = np.array([y_arr[sid_arr == i][0] for i in ids])
    tr = np.array([i in tr_ids for i in ids]); te = ~tr
    sc = StandardScaler().fit(V[tr])
    clf = LogisticRegression(max_iter=5000, C=C, random_state=0).fit(sc.transform(V[tr]), Y[tr])
    Z = sc.transform(V[te])
    return clf.decision_function(Z), Y[te], np.array(ids)[te]
...
for L in LAYERS:
    ...
    du, yu_, _ = probe_by_scenario(cache["OFF"][0][L], sid, y_urg, content_train, content_eval)
    a, b, c = (roc_auc_score(y_style[s_te], df_s), roc_auc_score(y_style[s_te], dc_s),
               roc_auc_score(yu_, du))
    base_tbl[L] = dict(style_final=a, style_cue=b, urgency_final=c)
    print(f"{L:>5} {a:>13.3f} {b:>12.3f} {b-a:>+12.3f} {c:>15.3f}")

style_unit = np.array([f"{m['dim']}|{m['phr']}" for m in meta])   # (dimension, phrasing) = the unit
...
print(f"  content probe : scenario-averaged, {len(content_eval)} held-out scenarios  <- the real n")
print("-"*104)
...
for L in LAYERS:
    ...
    off_u, yu_te, uid = probe_by_scenario(cache["OFF"][0][L], sid, y_urg, content_train, content_eval)
    on_u,  _,     _   = probe_by_scenario(cache["ON"][0][L],  sid, y_urg, content_train, content_eval)
    A = cluster_boot(off_f, on_f, y_style[s_te], su_te)
    B = cluster_boot(off_c, on_c, y_style[s_te], su_te)
    C = cluster_boot(off_u, on_u, yu_te, uid)
    pA, _ = cluster_perm(off_f, on_f, y_style[s_te], su_te)
    pC, _ = cluster_perm(off_u, on_u, yu_te, uid)
    ll = float('nan')
...
json.dump(dict(layers=LAYERS, base=base_tbl, deltas={str(k): v for k, v in tbl.items()},
          ...
          open("exp_D5_probe.json","w"), indent=2)
subprocess.run(["cp","exp_D5_probe.json",DRIVE+"/"],check=False)
print("\nsaved exp_D5_probe.json (per-item scores included: re-analysis needs no GPU)")
